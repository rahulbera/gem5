...
- `e2379e0b32` — final-review closure (neighbor-alias key fix, merge
  hygiene).
- `09c01d45f2` — 128-bit history snapshot + `--vtage-hist-lengths`
  (the history-series study's enabling commit).
...
- **History-series study (2026-08-04):** three variants on the
  16-checkpoint probe set (the MRN-era 14 + the two history-rich
  VTAGE winners): re-spaced-to-128 `{2,5,11,26,58,128}`,
  extended-to-128 `{2,4,8,16,32,64,128}`, and the within-64 density
  control `{2,4,6,11,20,36,64}`; the winner was promoted to all 190
  checkpoints in both scopes (`vtage_dense64`,
  `vtage_dense64_all`). Per-checkpoint probe numbers in
  `figs/vtage_hist_series_probe_per_checkpoint.csv`.
...
3. **The history-series follow-up (closing this report's original
   history-length next step): densifying *within* 64 bits of history
   beats reaching past it — `vtage_dense64` `{2,4,6,11,20,36,64}` is
   the best VTAGE measured, geomean 1.0212 loads-only / 1.0181
   all-insts at full scale.** The 16-checkpoint probe separated reach
   from density (`figs/vtage_hist_series.{png,pdf}`): both
   reach-to-128 variants fail to beat adding a component *inside* 64
   (re-spaced 6-component 1.0493, extended 7-component 1.0382, vs
   default 1.0422 and within-64 densify **1.0571**). The flat
   provider tail that motivated the study was real but misread: L=64's
   11% correct share came from patterns the coarse {32→64} jump was
   fumbling — mid-history *density*, not reach, was the deficiency.
   Worst-case reversal: `734.vpr_r.2.0` flips 0.9674 → **1.2241**
   (+26.5%), the largest single config response of the chapter. The
   dense-64 series is hereby the chapter's canonical VTAGE and the
   E-VTAGE report's comparison baseline.

4. **The loser signature inverts under VTAGE: flush volume no longer
...
5. **VTAGE fixes the majority of LVP's losers: 37 of 71 became
...
6. **The all-insts loser transition is mild — with one loud
...
7. **The history-correlation mechanism is proven in isolation:
...
8. **Worst-case anatomy (`734.vpr_r.2.0`, 0.9674 loads-only): the
...
9. **Measurement-integrity record: five defects were caught by review
...
2. **EVES per-class confidence rates [3]** — the designed follow-up
...
3. **The scheduling-side-effect class** — now the dominant residual
   ...
   next investigation.
4. **MRN + VP composition** — both mechanisms now have matched or
   better standalone gains and a coded, smoke-tested priority ladder;
   whether they compose or overlap is unmeasured.
5. **Threshold/geometry sweeps** — everything ran at untuned HPCA'14
