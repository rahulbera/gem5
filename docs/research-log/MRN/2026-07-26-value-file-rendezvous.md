...
**Composition with the legacy value-snapshot path.** The pre-existing MRN
value path (commit-trained, address-matched store-to-load value
communication) remains selectable alongside the model under a single-writer
rule: it serves only loads the value-file model claims nothing for. Two
knobs control the composition: `valueForwardConfThreshold` decouples its
confidence strictness from the model's, and `valueForwardOnUnconsumed`
widens its reach from "no value-file binding" to "value-file consumed
nothing" (below-confidence and dead-channel loads).
...
`fcf6bd1c70` (research-log update) · `e3d4b0e213` (legacy-path composition
knobs).
...
- **Composition study:** baseline MRN plus the legacy value-snapshot path,
  a 2x2 over fallback reach {unbound-only, unconsumed} x snapshot-path
  confidence threshold {14, 8}; 190 checkpoints per corner (760 runs),
  compared against baseline MRN per checkpoint.
...
5. **The loser tail is small, clustered, and mechanistically bimodal.** Only
   ...
   *correct* forwards perturb issue scheduling while the load still
   ...
   threshold fixes it.
6. **The legacy value-snapshot path is fully subsumable: half a billion
   correct forwards, zero net gain.** Composed under the single-writer rule
   (legacy serves only value-file-unclaimed loads), it fires on a huge
   population — 492 M forwards at 99.8% accuracy, nearly the model's own
   volume — and the geomean vs baseline MRN is 1.0000: its ~1.0 M
   mispredicts' flush tax (~70 M squashed instructions) cancels the gross
   benefit exactly. The out-of-window store-to-load population exists and
   is highly predictable, but it is not performance-critical —
   **criticality, not predictability, is the binding constraint**, the same
   lesson as the perturbation-dominated losers and the thesis of
   Constable [4]. Its confidence threshold (8 vs 14) moves the result by
   0.02% — noise. This de-risks deleting the legacy path outright.
7. **The model's refusals are informative: overriding them is negative.**
   Widening the legacy fallback to loads the value-file model bound but
   declined to serve (below-confidence or dead-channel) is worse than
   serving them with nothing: geomean 0.9984 vs baseline MRN, +0.5-0.9 M
   extra mispredicts, and heavy per-checkpoint churn (~62 win / ~80 lose).
   Shadow training had already judged those loads untrustworthy; a
   less-informed predictor overriding that judgment loses. Fallback should
   stop at "unclaimed", never extend to "declined".
8. **No meaningful Int/FP bias, by construction and by measurement.**
...
9. **Single-checkpoint results mislead in both directions.** gcc's +9.35%
...
1. **Delete the legacy value path and the `lsq_forward` correlator.** The
   composition study (Key Results 6-7) shows the legacy path earns nothing
   in any corner of the fallback-reach x threshold space; deletion is now
   empirically de-risked, and consolidation onto a single mechanism
   unblocks all further work.
2. **Store-write invalidation of self-bound cells** (Constable Condition-2
